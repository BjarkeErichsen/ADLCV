...
CONFIG = {
    ...
    "seed": 42,
    "concept_folder": "raw_baymax_concept", # TODO: Change this to your concept folder,  sec 1.1 Concept Preparation
}
    ...
    for epoch in range(num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(text_encoder):
                ...
                # 1️ Process images through VAE to get latents
                pixel_values = batch["pixel_values"].to(accelerator.device, dtype=weight_dtype)
                latents = vae.encode(pixel_values).latent_dist.sample()
                latents = latents * 0.18215  # Scale latent space
                
                # 2️ Add noise to latents using the noise scheduler
                ...
                # 3 Get text embeddings from the text encoder
                input_ids = batch["input_ids"].to(accelerator.device)
                encoder_hidden_states = text_encoder(input_ids).last_hidden_state

                # 4️ Predict noise with UNet and calculate loss
                noise_pred = unet(noisy_latents, timesteps, encoder_hidden_states).sample
                loss = F.mse_loss(noise_pred, noise)

                # 5️ Update only the embeddings for the placeholder token
                accelerator.backward(loss)
                optimizer.step()
                optimizer.zero_grad()

                # 6️ Save checkpoints at specified intervals
                if (global_step + 1) % CONFIG["save_steps"] == 0:
                    save_progress(text_encoder, placeholder_token_id, accelerator, os.path.join(output_dir, f"checkpoint_{global_step+1}.pt"))
...
def main():
    ...
    # 1. Load the trained pipeline from your output directory
    ...
    # 4. Create a test prompt that includes your placeholder token
    prompt = f"A <my-concept> standing in a futuristic city at sunset"

    # 5. Generate a small batch of images (2 samples is sufficient)
    images = pipeline(
        prompt,
        num_inference_steps=40,  # Higher values (30-50) give better quality but take longer
        guidance_scale=8.5,  # Values between 7-9 typically work well
        num_images_per_prompt=2,  # Generate 2 images
    ).images

    # 6. Arrange the generated images in a grid for easy viewing
    concept_grid = image_grid(images, rows=1, cols=2)

    # 7. Save the grid to your output directory
    concept_grid_path = os.path.join(CONFIG["output_dir"], "generated_concepts.png")
    concept_grid.save(concept_grid_path)

    print(f"Generated concept images saved to {concept_grid_path}")
