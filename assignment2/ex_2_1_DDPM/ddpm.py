...
class Diffusion:
    ...
    def get_betas(self, schedule='linear'):
        if schedule == 'linear':
            beta = torch.linspace(self.beta_start, self.beta_end, self.T)	
            return beta # HINT: use torch.linspace to create a linear schedule from beta_start to beta_end
        elif schedule == 'cosine':
            cosine = torch.cos(torch.linspace(0, 1, self.T) * 3.14159)
            return cosine
        # add your own (e.g. cosine)
        else:
            print("no scheduler")
            return torch.tensor([...] * self.T)  # Return a tensor with ellipsis values for unsupported schedules
