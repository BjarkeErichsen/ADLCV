...
class EncoderBlock(nn.Module):
    def __init__(self, embed_dim, num_heads, fc_dim=None, dropout=0.0, return_attention=False):
        super().__init__()
...
class ViT(nn.Module):
    def __init__(self, image_size, channels, patch_size, embed_dim, num_heads, num_layers,
                 ...
                 fc_dim=None, num_classes=2, plot_attention=False):
        ...
        self.to_patch_embedding = nn.Sequential(
            Rearrange('b c (h_koef h_p) (w_koef w_p) -> b c h_koef h_p w_koef w_p', h_p=patch_h, w_p = patch_w),
            Rearrange('b c h_koef h_p w_koef w_p -> b (h_koef w_koef) c h_p w_p'),             
            nn.LayerNorm([channels, patch_h, patch_w]),              
            Rearrange('b p c h_p w_p -> b p (c h_p w_p)'),
            nn.Linear(patch_dim, embed_dim),  
            nn.LayerNorm(embed_dim),
        )                               
         #b patch_dim channels patch_h patch_w
                    #
        ...
        for i in range(num_layers):
            transformer_blocks.append(
                EncoderBlock(embed_dim=embed_dim, num_heads=num_heads, fc_dim=fc_dim, dropout=dropout, return_attention=self.plot_attention))
