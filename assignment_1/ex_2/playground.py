...
def image_to_patches(image, patch_size, image_grid=True):
    ...
    if image_grid:
        ...
        HINT: B x c x H x W 
        to 
        B x C x num_patches_h x patch_h x num_patches_w x patch_w
        where H = num_patches_h * patch_h, W=num_patches_w * patch_w to
        """

        patches = rearrange(image, 'b c (h_koef h_p) (w_koef w_p) -> b c h_koef h_p w_koef w_p', h_p=patch_h, w_p = patch_w)
        ...
        """
        patches = rearrange(patches, 'b c h_koef h_p w_koef w_p -> b (h_koef w_koef) c h_p w_p')
        ...
        """
        patches = rearrange(image, 'b c (h_koef h_p) (w_koef w_p) -> b (h_koef w_koef) (c h_p w_p)', h_p=patch_h, w_p = patch_w)
