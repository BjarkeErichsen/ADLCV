...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        attention_logits = torch.matmul(queries, keys.transpose(-2, -1))  # Shape: [b, h, seq_len, seq_len]
        # attention_logits = queries*keys # multiply queries and keys
        attention_logits = attention_logits * self.scale
        attention = nn.Softmax(dim=-1)(attention_logits) # softmax on attention
        # attention = rearrange(attention, 'b h s1 s2 -> (b h) s1 s2')  # Match batch and num_heads
